...
The `hooks/` folder contains executable scripts that customize repository behavior. Hooks are divided into **client-side** and **server-side** categories, with plugin architecture for extensibility.
...
async function handleGetRequest(path, ctx)
  Returns: JSX element or null if path doesn't match

// QUERY hook handler  
async function handleQuery(query, options, ctx)
  Returns: {items, total, page, source} or null
```
