...
async function registerTemplateThemeStyles() {
    ...
    try {
        ...
        // Fallback: if YAML helper missing, no-op to avoid breaking UI
        ...
        async function loadContent() {
            try {
                ...
                if (tmdbPlugin && typeof tmdbPlugin.handleGetRequest === 'function') {
                    pluginResult = await tmdbPlugin.handleGetRequest(path, { React, createElement: React.createElement, MarkdownRenderer });
                    if (pluginResult) {
                ...
                if (ytsPlugin && typeof ytsPlugin.handleGetRequest === 'function') {
                    pluginResult = await ytsPlugin.handleGetRequest(path, { React, createElement: React.createElement, MarkdownRenderer });
                    if (pluginResult) {
