// Global client script loaded as an ES module on every HTML page.
// Defines a <relay-dropzone> Web Component that enables drag & drop uploads
// via PUT requests to the Relay server. The server will validate uploads
// against relay.yaml (allowedPaths, metaSchema) and reject invalid ones.

class RelayDropzone extends HTMLElement {
  constructor() {
    super();
...
// Progressive enhancement: if server didn't wrap, wrap main content now
window.addEventListener('DOMContentLoaded', () => {
  if (!document.querySelector('relay-dropzone')) {
    const article = document.querySelector('article.markdown-body')
      || document.querySelector('.markdown-body');
    if (article && article.parentElement) {
      const wrapper = document.createElement('relay-dropzone');
...
// Optional: basic drag-over styling
const style = document.createElement('style');
style.textContent = `
relay-dropzone.relay-dragover, relay-dropzone .relay-dragover {
